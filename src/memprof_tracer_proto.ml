  ...
  let tracker : (_,_) Gc.Memprof.tracker = {
    alloc_minor = (fun info ->
      if lock_tracer s then begin
        match Proto.Writer.put_alloc_with_raw_backtrace trace (Proto.Timestamp.now ())
                ~length:info.size
    ...
    promote = (fun id ->
      if lock_tracer s then
        match Proto.Writer.put_promote trace (Proto.Timestamp.now ()) id with
        | () -> unlock_tracer s; Some id
        | exception e -> mark_failed s e; None
      else None);
    dealloc_minor = (fun id ->
      if lock_tracer s then
        match Proto.Writer.put_collect trace (Proto.Timestamp.now ()) id with
        | () -> unlock_tracer s
...
let ext_free id =
  ...
  | Some s ->
    if lock_tracer s then begin
      match
        Proto.Writer.put_collect s.trace (Proto.Timestamp.now ()) id
      with
      | () -> unlock_tracer s; ()
      | exception e -> mark_failed s e; ()
    end